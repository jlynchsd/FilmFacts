...
class GetRatedTvShowSeasonUseCaseTest {
    ...
    fun setup() {
        ...
        coEvery {
            ...
            stubTvShowDetails(seasons = listOf(
                stubTvShowSeasonDetails(voteAverage = 1f),
                stubTvShowSeasonDetails(voteAverage = 2f),
                stubTvShowSeasonDetails(voteAverage = 3f),
                stubTvShowSeasonDetails(voteAverage = 4f)
            )),
        )
    ...
    fun `When unable to get tv show details then returns null`() = runTest {
        coEvery {
            getTvShowDetails(any(), any(), any(), any())
        } returns emptyList()
    ...
    @Test
    fun `When not enough distinct seasons then returns null`() = runTest {
        coEvery {
            getTvShowDetails(any(), any(), any(), any())
        } returns listOf(
            stubTvShowDetails(seasons = listOf(
    ...
    @Test
    fun `When not enough rated seasons then returns null`() = runTest {
        coEvery {
            getTvShowDetails(any(), any(), any(), any())
        } returns listOf(
            stubTvShowDetails(seasons = listOf(
                stubTvShowSeasonDetails(voteAverage = 0f),
                stubTvShowSeasonDetails(voteAverage = 1f),
                stubTvShowSeasonDetails(voteAverage = 3f),
                stubTvShowSeasonDetails(voteAverage = 4f)
            )),
    ...
    @Test
    fun `When using top rated strategy then returns result with top rated answer`() = runTest {
        val useCase = getUseCase(testScheduler)

        val result = useCase.getPrompt(null, GetRatedTvShowSeasonUseCase.RatedStrategies.TOP_RATED) as UiImagePrompt

        Assert.assertEquals("4.0", result.entries.first { it.isAnswer }.data)
    }

    @Test
    fun `When using worst rated strategy then returns result with worst rated answer`() = runTest {
        val useCase = getUseCase(testScheduler)

        val result = useCase.getPrompt(null, GetRatedTvShowSeasonUseCase.RatedStrategies.WORST_RATED) as UiImagePrompt

        Assert.assertEquals("1.0", result.entries.first { it.isAnswer }.data)
    }

    private fun getUseCase(testScheduler: TestCoroutineScheduler) =
        GetRatedTvShowSeasonUseCase(
