...
class GetLongestRunningTvShowUseCaseTest {
    ...
    fun setup() {
        ...
        coEvery {
            ...
            stubTvShowDetails(numberOfEpisodes = 1),
            stubTvShowDetails(numberOfEpisodes = 2),
            stubTvShowDetails(numberOfEpisodes = 3),
            stubTvShowDetails(numberOfEpisodes = 4)
        )
    ...
    fun `When valid data is provided then returns result with correct answer`() = runTest {
        ...
        Assert.assertEquals("4 Episodes", result.entries.first { it.isAnswer }.data)
    }
